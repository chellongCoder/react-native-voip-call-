...
class RNVoipCallNativeModule {
  ...
  displayIncomingCall = (options) =>
    new Promise((resolve, reject) => {
      ...
      if (!IsIOS) {
        const androidOptions = this.getDefualtValues(options.android);
        RNVoipCall.displayIncomingCall({
          callerId: options.callerId,
          ...androidOptions,
        });
      } else {
        const iosOptions = this.getDefualtValues(options.ios);
  ...
  playRingtune = (name, loop) => {
    if (!IsIOS) {
      const fileName = name ? name : 'ringtune';
      const looping = loop || loop === false ? loop : false;
